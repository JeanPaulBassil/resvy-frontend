...
export default function FloorPlan({
  ...
  const renderFloorPlanContent = () => (
    <>
          ...
          <div className="w-28 h-10 border-2 border-[#75CAA6]/60 rounded-t-lg bg-[#75CAA6]/10 flex items-center justify-center shadow-md">
            <div className="text-xs text-[#75CAA6] font-medium flex items-center gap-1.5">
              <svg
                ...
                strokeLinejoin="round"
              >
                <path d="M9 6v11a2 2 0 0 0 2 2h9a2 2 0 0 0 2-2V6a2 2 0 0 0-2-2h-9a2 2 0 0 0-2 2z"></path>
                <path d="M3 10v6a2 2 0 0 0 2 2h2"></path>
                <path d="M10 8V6a2 2 0 0 0-2-2H6"></path>
                <path d="M7 8v1"></path>
              </svg>
              Entrance
            </div>
      ...
      {/* Animation overlay for merge/unmerge animations */}
      <AnimatePresence>
        {animatingMerge && (
          <motion.div
      ...
      <AnimatePresence>
        {mergeAnimations.map((anim) => {
          if (!anim.initialPositions) return null;

          const originTable = tables.find((t) => t.id === anim.sourceTableId);
          const targetTable = tables.find((t) => t.id === anim.targetTableId);

          if (!originTable || !targetTable) return null;

          // Add sparkle effects along the path
          const sparkleCount = 10;
          const sparkles = Array.from({ length: sparkleCount }, (_, i) => {
            const progress = i / sparkleCount;
            const delay = progress * 0.3; // Stagger the animations

            // Calculate position along the path
            const originX =
              anim.state === TableAnimationState.MERGING
                ? originTable.x + 40 // center of table
                : targetTable.x + 40;

            const originY =
              anim.state === TableAnimationState.MERGING ? originTable.y + 40 : targetTable.y + 40;

            const targetX =
              anim.state === TableAnimationState.MERGING ? targetTable.x + 40 : originTable.x + 40;

            const targetY =
              anim.state === TableAnimationState.MERGING ? targetTable.y + 40 : originTable.y + 40;

            // Use quadratic bezier curve for more interesting motion
            const controlX = (originX + targetX) / 2 + 20;
            const controlY = (originY + targetY) / 2 - 20;

            // Parametric equation for quadratic bezier curve
            const t = anim.state === TableAnimationState.MERGING ? progress : 1 - progress;

            const x = (1 - t) * (1 - t) * originX + 2 * (1 - t) * t * controlX + t * t * targetX;
            const y = (1 - t) * (1 - t) * originY + 2 * (1 - t) * t * controlY + t * t * targetY;

            return (
              <motion.div
                key={`sparkle-${anim.sourceTableId}-${anim.targetTableId}-${i}`}
                className="absolute w-3 h-3 rounded-full bg-indigo-400 shadow-lg z-20 pointer-events-none"
                style={{
                  left: x - 6,
                  top: y - 6,
                  background:
                    'radial-gradient(circle, rgba(99,102,241,1) 0%, rgba(165,180,252,0.8) 70%, rgba(224,231,255,0) 100%)',
                }}
          ...
          return <div key={`sparkles-${anim.sourceTableId}-${anim.targetTableId}`}>{sparkles}</div>;
        })}
      </AnimatePresence>
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <div className="bg-white/90 backdrop-blur-sm p-4 rounded-xl shadow-lg border border-gray-200 max-w-xs text-center">
            <svg
              ...
              strokeLinecap="round"
              strokeLinejoin="round"
              className="mx-auto mb-2 text-[#75CAA6]"
            >
      ...
      {isFullScreen && (
        <div className="absolute top-4 right-4 bg-white/80 backdrop-blur-sm rounded-lg shadow-sm border border-gray-200 flex flex-col">
          ...
          <button className="p-1.5 hover:bg-gray-100 rounded-b-lg" onClick={zoomOut}>
